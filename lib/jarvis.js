...
export function jarvisRespond(query, articles) {
  ...
  // ---- Agradecimiento ----
  if (/(gracias|thank|genial|perfecto|excelente|buenisimo|chevere|brutal|tremendo)/.test(q)) {
    return {
      reply: rnd([
        'Para eso estoy, FER.',
  ...
  // ---- Búsqueda libre ----
  const clean = q.replace(/^(busca|buscar|encuentra|hay algo de|noticias de|sobre|que hay de|info de|dame|muestrame)\s+/i, '').trim();
  if (clean.length >= 3) {
    const hits = pick(
      articles
        .filter((a) => norm(a.title + ' ' + a.description + ' ' + a.source).includes(clean))
        .sort((a, b) => b.importance - a.importance),
      5
    );
    if (hits.length) {
      return {
        reply: `Encontré ${hits.length} resultado${hits.length === 1 ? '' : 's'} para «${clean}», FER:`,
